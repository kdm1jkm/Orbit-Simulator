...
OBJECT_EXAMPLES: Dict[ObjectExample, List[Object]] = {
    ...
    ObjectExample.SIMPLIFIED_SOLAR_SYSTEM: [
        ...
        Object(
            HEAVY_MASS,
            np.array([10000, 0]),
            np.array([100, (G * HEAVIER_MASS / 10000) ** 0.5]),
        ),
        Object(
            LIGHT_MASS,
            np.array([10100, 0]),
            np.array(
                [100, (G * HEAVY_MASS / 100) ** 0.5 + (G * HEAVIER_MASS / 10000) ** 0.5]
            ),
        ),
    ],
    ...
    ObjectExample.PERFECT_CIRCLE: [
        ...
        Object(
            LIGHT_MASS, np.array([500, 0]), np.array([0, (G * HEAVY_MASS / 500) ** 0.5])
        ),
    ],
}
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == pygame.BUTTON_LEFT:
                    if make_object:
                        pos = to_original_coord(
                            np.array(pygame.mouse.get_pos()), screen_center, zoom
                        )
                        prepare_object = True

                elif event.button == pygame.BUTTON_RIGHT:
                    mouse_start = to_original_coord(
                        np.array(pygame.mouse.get_pos()), screen_center, zoom
                    )
                    moving_screen = True
        ...
        if moving_screen:
            screen_center = screen_center + (
                to_original_coord(np.array(pygame.mouse.get_pos()), screen_center, zoom)
                - mouse_start
            )
            mouse_start = to_original_coord(
                np.array(pygame.mouse.get_pos()), screen_center, zoom
        ...
        if make_object:
            pygame.draw.circle(
                screen, BLACK, screen_pos, int(next_mass ** (1 / 14) * zoom) + 2
            )
            pygame.draw.line(
                screen, RED, screen_pos, pygame.mouse.get_pos(), int(3 * zoom) + 1
            )
        if make_object and len(objects) > 0:
            ...
            for sort in objects:
                ...
                mass_sum = (
                    mass_sum[0] + sort.mass / r,
                    mass_sum[1] + sort.coord * sort.mass / r,
                )
            ...
            pygame.draw.circle(
                screen, RED, screen_pos, int((2 * G * mass_sum[0] * r) ** 0.5 * zoom), 3
            )
            pygame.draw.line(
                screen,
                BLUE,
                to_screen_coord(mass_center, screen_center, zoom),
                screen_pos,
                3,
            )
            ...
            guide1 = np.array(
                [
                    ...
                    + (G * mass_sum[0] * r) ** 0.5
                    / r
                    * (mass_sum[1][1] / mass_sum[0] - pos[1]),
                    pos[1]
                    - (G * mass_sum[0] * r) ** 0.5
                    / r
                    * (mass_sum[1][0] / mass_sum[0] - pos[0]),
                ]
            )
            guide2 = np.array(
                [
            ...
            pygame.draw.line(
                screen,
                BLUE,
                to_screen_coord(guide1, screen_center, zoom),
        ...
        for sort in objects:
            ...
            pygame.draw.circle(
                screen,
                color,
                to_screen_coord(sort.coord, screen_center, zoom),
            ...
            if show_physical_quantity:
                pygame.draw.line(
                    ...
                    RED,
                    to_screen_coord(sort.coord, screen_center, zoom),
                    to_screen_coord(sort.coord + sort.velocity, screen_center, zoom),
                    int(3 * zoom) + 1,
                )
            ...
            for sort in objects:
                ...
                if show_physical_quantity:
                    pygame.draw.line(
                        ...
                        BLUE,
                        to_screen_coord(sort.coord, screen_center, zoom),
                        to_screen_coord(sort.coord + a, screen_center, zoom),
                        int(3 * zoom) + 1,
                    )
                ...
                if r < o1.mass ** (1 / 15) + o2.mass ** (1 / 15):
                    new_object.append(
                        Object(
                            o1.mass + o2.mass,
                            (o1.coord + o2.coord) / 2,
                            (o1.velocity * o1.mass + o2.velocity * o2.mass)
                            / (o1.mass + o2.mass),
                        )
                    )
                    objects[i] = None
